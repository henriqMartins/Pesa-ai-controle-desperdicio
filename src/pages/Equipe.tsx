...
function ModalFuncionario({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center anim-fade px-4" style={{ background: 'rgba(0,0,0,.65)', backdropFilter: 'blur(3px)' }} onClick={onClose}>
      <div className="anim-pop w-full max-w-sm rounded-2xl" style={{ background: '#15100b', border: '1px solid rgba(255,220,180,.1)', boxShadow: '0 20px 50px rgba(0,0,0,.5)' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          <button type="button" onClick={() => setAtivo((v) => !v)} className="flex w-full items-center justify-between rounded-xl px-4 py-3" style={{ background: '#1c160f', border: '1px solid rgba(255,220,180,.1)' }}>
            <span className="text-sm font-bold text-white/70">Ativo</span>
            <span className="relative h-6 w-11 rounded-full transition-colors" style={{ background: ativo ? GRAD : 'rgba(255,255,255,.15)' }}>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 px-4 py-8">
      <div className="overflow-hidden rounded-2xl" style={{ background: '#1c160f', border: '1px solid rgba(255,220,180,.07)' }}>
        <div className="flex items-center justify-between px-4 py-3.5" style={{ borderBottom: '1px solid rgba(255,220,180,.06)' }}>
        ...
        {!loading && funcionarios.map((f, i) => (
          <div key={f.id} className="flex items-center gap-3 px-4 py-3.5" style={{ borderBottom: '1px solid rgba(255,220,180,.05)' }}>
            <div className="flex h-10 w-10 flex-none items-center justify-center rounded-full text-sm font-extrabold text-white" style={{ background: f.ativo ? AVATAR_COLORS[i % AVATAR_COLORS.length] : 'rgba(255,255,255,.12)' }}>
              {f.nome.charAt(0).toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
              <span className={`font-semibold ${f.ativo ? 'text-white' : 'text-white/40 line-through'}`}>{f.nome}</span>
              <div className="text-xs font-semibold" style={{ color: f.papel === 'gestor' ? '#ff8a4c' : 'rgba(255,255,255,.4)' }}>{f.papel === 'gestor' ? 'Gestor' : 'Funcionário'}</div>
            </div>
            ...
            <button onClick={() => setModal({ aberto: true, funcionario: f })} className="flex h-8 w-8 items-center justify-center rounded-lg text-white/45 hover:text-white" style={{ border: '1px solid rgba(255,220,180,.12)' }} aria-label="editar">✎</button>
          </div>
        ))}
      </div>
